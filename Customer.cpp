...
void loadPlaylistFromFile(ifstream& file, Customer& customer, DoublyLinkedList& globalPool) {
    string uname, pName;
    int songCount, songId;
 
    // Step 1: try to read username, playlist name, and song count from the file 
    if (file >> uname >> pName >> songCount) {
        customer.playlistName = pName;  //restore the saved playlist name
        customer.myPlaylist.clear();    //make sure start from an empty playlist
        ...
        for (int i = 0; i < songCount; i++) {
            file >> songId;
            customer.myPlaylist.addSong(songId, globalPool); // Shinru's overload that looks up by ID
        }
    }
}
...
void Customer::addSongToPlaylist(DoublyLinkedList& globalPool) {
    ...
    try {
	    ...
	    if (cin.fail()) {
	        cin.clear();
	        cin.ignore(1000, '\n');
	        throw CustomerException("Please enter a valid number.");
	    }
	    ...
	    while (curr != nullptr) {
	        if (curr->data.id == id) {
	            throw CustomerException(
	            	"Song ID " + to_string(id) + " is already in your playlist.");
	        }
	        curr = curr->next;
	    }
	    ...
	    if (result == -1) {
	    	delete [] arr;
	       throw CustomerException (
	       "[Error] Song ID " + to_string(id) + 
	       " does not exist in the music pool."
		   );
	    }
...
void Customer::displayPlaylist() {
    ...
    while (curr != nullptr) {
        cout << num << ". "
             ...
             << curr->data.artist << " | "
             << curr->data.genre << " | "
             << "Plays: " << curr->data.popularity << " | "
             << curr->data.duration << "s" << endl;
...
void Customer::searchInPlaylist() {
    ...
    if (option == 1) {
        ...
        // sort by ID first before binary search — using Member A's own quickSortSong (criteria 3)
        ...
        // sort by title first before binary search — using Member A's own quickSortSong (criteria 1)
...
User* registerCustomer() {
    ...
    // make a temporary Customer just so we can reuse Shinru's saveUserToFile()
    Customer* newUser = new Customer(uname, pass);
...
User* loginCustomer(DoublyLinkedList& globalPool) {
    ...
    if (custPtr != nullptr) {
        ...
        if (playlistFile.is_open()) {
            ...
            while (true) {
                // remember where we are in the file right now
                streampos pos = playlistFile.tellg();
                // stop if we've reached the end of the file
                if (!(playlistFile >> lineUser)) break;
                ...
                if (lineUser == uname) {
                    ...
                    // this line belongs to a different user, skip over it
                    string pName;
                    int count;
                    playlistFile >> pName >> count;
    ...
    if (inFile.is_open()) {
        ...
        while (inFile >> lineUser) {
            ...
            if (lineUser == customer.getUsername()) {
                ...
                // write the updated playlist instead
                savePlaylistToFile(tempFile, customer);
                saved = true;
            }
            else {
                // not our customer, so just copy this line over exactly as it was
                tempFile << lineUser << " " << pName << " " << count << " ";
                for (int i = 0; i < count; i++) {
                    int id;
                    inFile >> id;
                    tempFile << id << " ";
...
int main() {
    ...
    while (true) {
        cout << "\n========================================" << endl;
        cout << "  MUSIC PLAYLIST ORGANIZER (Customer Test)" << endl;
        cout << "========================================" << endl;
